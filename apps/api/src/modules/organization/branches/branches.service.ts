...
// mapPgError. Audit stamping arrives with auth (Phase 5). See docs/API_AND_AUTH_PLAN.md, Phase 1.
...
export class BranchesService {
  ...
  async update(id: string, input: UpdateBranch): Promise<Branch> {
    let branch: Branch | undefined;
    try {
      branch = await this.repository.update(id, input);
    } catch (error) {
